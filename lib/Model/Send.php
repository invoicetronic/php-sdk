...
class Send implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    public const LATEST_STATE_INVIATO = 'Inviato';
    public const LATEST_STATE_CONSEGNATO = 'Consegnato';
    public const LATEST_STATE_NON_CONSEGNATO = 'NonConsegnato';
    public const LATEST_STATE_SCARTATO = 'Scartato';
    public const LATEST_STATE_ACCETTATO_DAL_DESTINATARIO = 'AccettatoDalDestinatario';
    public const LATEST_STATE_RIFIUTATO_DAL_DESTINATARIO = 'RifiutatoDalDestinatario';
    public const LATEST_STATE_IMPOSSIBILITA_DI_RECAPITO = 'ImpossibilitaDiRecapito';
    public const LATEST_STATE_DECORRENZA_TERMINI = 'DecorrenzaTermini';
    public const LATEST_STATE_ATTESTAZIONE_TRASMISSIONE_FATTURA = 'AttestazioneTrasmissioneFattura';
    ...
    {
        return [
            self::LATEST_STATE_INVIATO,
            self::LATEST_STATE_CONSEGNATO,
            self::LATEST_STATE_NON_CONSEGNATO,
            self::LATEST_STATE_SCARTATO,
            self::LATEST_STATE_ACCETTATO_DAL_DESTINATARIO,
            self::LATEST_STATE_RIFIUTATO_DAL_DESTINATARIO,
            self::LATEST_STATE_IMPOSSIBILITA_DI_RECAPITO,
            self::LATEST_STATE_DECORRENZA_TERMINI,
            self::LATEST_STATE_ATTESTAZIONE_TRASMISSIONE_FATTURA,
        ];
    }
    ...
    public function __construct(?array $data = null)
    {
        ...
        $this->setIfExists('latest_state', $data ?? [], null);
    ...
    */
    ...
    public function listInvalidProperties()
    {
        ...
        if (!is_null($this->container['latest_state']) && !in_array($this->container['latest_state'], $allowedValues, true)) {
            $invalidProperties[] = sprintf(
                "invalid value '%s' for 'latest_state', must be one of '%s'",
                $this->container['latest_state'],
                implode("', '", $allowedValues)
            );
        }
     ...
     * Sets id
     *
     * @param int|null $id Unique identifier. For POST requests, leave it at `0` — the server will assign one automatically. For PUT requests, set it to the id of the record you want to update.
     *
     * @return self
     */
     ...
     * Sets user_id
     *
     * @param int|null $user_id User id. It is set automatically based on the authenticated user.
     *
     * @return self
     */
     ...
     * Sets company_id
     *
     * @param int|null $company_id Company id. It is set automatically based on the VAT number extracted from the invoice payload (the sender for `send`, the recipient for `receive`).
     *
     * @return self
     */
     ...
     * Sets identifier
     *
     * @param string|null $identifier SDI identifier. This is set by the SDI and is guaranteed to be unique within the SDI system.
     *
     * @return self
     */
     ...
     * Sets file_name
     *
     * @param string|null $file_name Xml file name. If not provided on send, it will be auto-generated.
     *
     * @return self
     */
     ...
     * Sets payload
     *
     * @param string $payload Xml payload. This is the actual xml content, as string. On send, it can be base64 encoded. If it's not, it will be encoded before sending. It is guaranteed to be encrypted at rest.
     *
     * @return self
     */
     ...
     * Sets latest_state
     *
     * @param string|null $latest_state Current SDI state of the invoice. Reflects the most recent update received from SDI. Null when no update has been received yet.
     *
     * @return self
     */
    public function setLatestState($latest_state)
    {
        if (is_null($latest_state)) {
            array_push($this->openAPINullablesSetToNull, 'latest_state');
        } else {
            $nullablesSetToNull = $this->getOpenAPINullablesSetToNull();
            $index = array_search('latest_state', $nullablesSetToNull);
            if ($index !== FALSE) {
                unset($nullablesSetToNull[$index]);
                $this->setOpenAPINullablesSetToNull($nullablesSetToNull);
            }
